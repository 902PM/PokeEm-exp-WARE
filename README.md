...
→ただし、戦法なんてAIにはなく、ほぼ初手で使ってくる。ので、テラスタルがただのタイプチェンジでしかない。<br>
...
- アダン勝利後、ルネシティ左上の家のわざマシン31を受け取ろうとするとクラッシュする。<br>→何度か試したが再現性がない（おそらく予想として、バッファ周り。でクラッシュしない回はエンカウント等をして、バッファ解消されているから？）
...
- バトルフロンティア限定？でなんかスタートからフィールド貼ってくるヤツがいる（現在タワーで確認）
<br>→一般トレーナーはこちら側がわざと設定している。使ってくるポケモンが◯◯メイカーでもないので、バグ？？

- 未だにZワザで『-』になっている。ForceGimmick入れたけど、いらんかもしれん。これが邪魔している気がする。
<br>→そも、テラスタルとダイマックスはフラグでするしないが決まってるっぽいから、いちいち設定してあげればよくね？
<br>→メガシンカとZワザは必要アイテムを持ってれば、普通に使ってくる。だからForceする必要がない。
<br>たぶんZワザだけ正しいワザを選ぶ必要があるから、こうなっている。
...
- AIは確1の時、先制技打ってチョロダメを与えようとする。なので、先制技控えして対策。ラス1でもそれするのやめよ？

- 敵に対して『こうかなし』状態でこだわっているのに、引かないでつっぱるAI。この状況じゃなくても、なんか『こうかなし』打ってくる時があるっぽい？
<br>→ファクトリーの低周回層にフラグが設定されていなかったせいな気も、要検証。

- ダイマックスそのもののギミックがズルい。HP増えて固いし、キョダイワザならスリップダメージ出すし。ダイジェットとかズルじゃん。それにマルスケはズルだろ。
<br>→レベル50環境はちょっとしんどいかも。普通に倒しきれない。
<br>→でもAIがバカなので、変化技いれると、ダイウォールでターン無駄にしてくれる。
<br>→アリーナダイマとか正直ズルいと思います。ドームも２匹な関係できびいかも。
