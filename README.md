...
<br>一般トレーナーはこちら側がわざと設定している。◯◯メイカーでもないので、バグ？？
...
- (とくに113ばんどうろのハジツゲの前で起こる現象)上下に行ったり来たりするトレーナーと視線エンカウントする時に、反対向いて迫ってきて、そのまま会話して戦闘。（おそらく、NPCをロックできていない）
→フォーク元のIssuesにこれと同じ現象があったので、バグ確定。

- ウインドウで表示されるテキストが原作JPの位置と厳密には違う。(特にレポートは桁数が変わる関係でズレまくる。)
→レポート画面、対戦成績画面、きのみブレンダー結果画面、戦闘のUI、パーティ画面のUIなど、見比べるとちょっとズレてたり、フォントが違う。

- 乗船する時に、連れ歩きポケモンが置いていかれる（フォーク元のバグというか、仕様）
<br>→これは実害ないので、あちら側が直さない限り、触れない。
...
<br>→とりま、フロンティアで出たり出なかったり、そもそもプレイしているセーブデータのバージョンが低いとかもあったりするので、さらなる検証が必要。

- ダイマックスのバランスがイかれてる。HPが増えて、耐えられて、殴られて、やられる。メガシンカじゃ、倒しきれない。
<br>→ダイママルスケカイリューはヤバいだろ。フォーク元がダイマックス技の計算式の更新が行われてからは、割といけるか？って感じ。
