...
// IV/EV settings
#define P_SUMMARY_SCREEN_IV_EV_INFO      FALSE  // TRUEに設定すると、ステータス画面で実数値、個体値（IV）、努力値（EV）の表示を切り替えられるようになります。
...
#define P_SUMMARY_SCREEN_IV_EV_VALUES    FALSE  // TRUE の場合、文字での表示の代わりに実際の個体値が表示されます。
/*
LETTER GRADE GUIDE:
...
#define P_ENABLE_MOVE_RELEARNERS         FALSE    // TRUEに設定すると、タマゴ技、わざマシン、および教え技の技思い出し機能が有効になります（具体的な設定やフラグについては下記を参照してください）。
#define P_SORT_MOVES                     FALSE    // TRUEの場合、技の思い出し屋のリストにあるすべての技をアルファベット順に並べ替えます。

// Level up Relearner
#define P_PRE_EVO_MOVES                  FALSE    // TRUEの場合、そのポケモンは進化前の段階で覚える技を覚えることができます。
#define P_ENABLE_ALL_LEVEL_UP_MOVES      FALSE    // TRUEの場合、レベルに関係なく、そのポケモンはレベルアップで覚えるすべての技を覚えることができます。

// TM Relearner
#define P_TM_MOVES_RELEARNER             FALSE    // TRUEの場合、『わざおもいだし』の機能を有効にします。
#define P_ENABLE_ALL_TM_MOVES            FALSE    // TRUEの場合、バッグに入っているかどうかにかかわらず、そのポケモンは対応するすべてのわざマシンを覚えることができます。
