...
#define SHOULD_SWITCH_CHOICE_LOCKED_PERCENTAGE                      100 // 「変化技」に固定されている場合のみ
...
#define AI_BAD_SCORE_THRESHOLD                                  90 // 『いれかえ』を判断する際、この閾値を下回る技評価は『不可』とみなされます。
...
#define ENABLE_RECOVERY_THRESHOLD                               60 // SHOULD_RECOVER_CHANCEが有効になるHPの割合
...
#define EXPLOSION_HIGHER_HP_THRESHOLD                           90 // AIが自爆する確率が0%となるHPの割合（これ以上の場合は自爆しない）。それ未満の場合は、この閾値と最低閾値の間で確率が変動する。
