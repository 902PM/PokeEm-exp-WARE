...
#define B_CRIT_MULTIPLIER           GEN_LATEST // 第6世代以降、急所ヒット時のダメージ倍率は2倍ではなく1.5倍になります。
...
#define B_PARALYSIS_CHANCE          GEN_LATEST // Champions以降、麻痺による行動不能の確率は25%ではなく12.5%です。

// 経験値設定
#define B_EXP_CATCH                 GEN_LATEST // 第6世代以降、ポケモンは捕獲することでも経験値を得られるようになりました。
#define B_TRAINER_EXP_MULTIPLIER    GEN_LATEST // 第7世代以降、トレーナー戦で獲得できる経験値に1.5倍の補正はかからなくなりました。
...
#define B_LEVEL_UP_NOTIFICATION     GEN_LATEST // 第9世代以降、ポケモンがレベルアップに必要な経験値を十分に獲得し、複数回レベルアップする場合でも、メッセージは1回しか表示されません。
...
#define B_MAX_LEVEL_EV_GAINS        GEN_LATEST // 第5世代以降、レベル100のポケモンも通常通り努力値を獲得できます。
#define B_RECALCULATE_STATS         GEN_LATEST // 第5世代以降、戦闘に参加したポケモンのステータスが、各戦闘の終了時に再計算されます。

// ダメージ設定
#define B_BURN_DAMAGE               GEN_LATEST // 第7世代以降、やけどによるダメージは最大HPの1/8ではなく1/16になりました。これは「しもやけ」にも同様に適用されます。
...
#define B_HIDDEN_POWER_DMG          GEN_LATEST // 第6世代以降、「めざめるパワー」の威力は常に60に固定されるようになりました。それ以前は、ポケモンの個体値によって威力が決まっていました。
...
#define B_EXPLOSION_DEFENSE         GEN_LATEST // 第5世代以降、「じばく」や「だいばくはつ」は、相手の防御を半分にしません。
...
#define B_STRUGGLE_RECOIL           GEN_LATEST // 第4世代以降、「わるあがき」の反動ダメージは使用者の最大HPの1/4です。第2・3世代では与えたダメージの1/4、第1世代では与えたダメージの1/2となります。
#define B_RETURN_FRUSTRATION_DMG    GEN_LATEST // 第3世代以降、「おんがえし」と「やつあたり」は最低でも1のダメージを与えます。一方、第2世代では、威力が0になるなつき度の場合、ダメージは与えられません。

// タイプ設定
#define B_GHOSTS_ESCAPE             GEN_LATEST // 第6世代以降、「かげふみ」などの特性や「くろいまなざし」などの技は、ゴーストタイプのポケモンに対しては効果がありません。また、野生のポケモンとのバトルから逃げることも可能です。
#define B_PARALYZE_ELECTRIC         GEN_LATEST // 第6世代以降、でんきタイプのポケモンはまひ状態になりません。
#define B_POWDER_GRASS              GEN_LATEST // 第6世代以降、くさタイプのポケモンは粉・胞子技を無効化します。
#define B_UPDATED_TYPE_MATCHUPS     GEN_LATEST // タイプ相性を更新しました。詳細は src/data/types_info.h を参照。
#define B_PRANKSTER_DARK_TYPES      GEN_LATEST // 第7世代以降、「いたずらごころ」によって優先度が上がった変化技は、あくタイプのポケモンには無効です。
#define B_SHEER_COLD_IMMUNITY       GEN_LATEST // 第7世代以降、こおりタイプは「ぜったいれいど」を無効化します。
#define B_ROOST_PURE_FLYING         GEN_LATEST // 第5世代以降、「はねやすめ」は純粋なひこうタイプのポケモンをノーマルタイプに変化させます。
#define B_STATUS_TYPE_IMMUNITY      GEN_LATEST // 第1世代では、攻撃技と同じタイプを持つポケモンは、その技の追加効果である「まひ」「こおり」「やけど」を受けませんでした。
#define B_FIXED_DMG_IGNORES_TYPE    GEN_LATEST // 第1世代では、固定ダメージ技は、タイプの相性や無効化の影響を受けません。
...
#define B_FREEZE_TURNS              GEN_LATEST // Championsより前は、凍結状態が解除される確率が20%でした。
...
#define B_HIDDEN_POWER_COUNTER      GEN_LATEST // 第4世代以前、「カウンター」と「ミラーコート」は、タイプにかかわらず「めざめるパワー」を物理技として扱います。
#define B_MODERN_TRICK_CHOICE_LOCK  GEN_LATEST // 第5世代以降、「こだわり」系アイテム同士を「トリック」や「すりかえ」で交換した場合、その技の使い手は別の技を選択でき、その後はその技に固定されることになります。
...
#define B_BLIZZARD_HAIL             GEN_LATEST // 第4世代以降、あられ状態の際、「ふぶき」必中。
#define B_SHEER_COLD_ACC            GEN_LATEST // 第7世代以降、使用者がこおりタイプでない場合、「ぜったいれいど」の基本命中率は20%に低下します。

// ステータス変化技の設定
#define B_FELL_STINGER_STAT_RAISE   GEN_LATEST // 第7世代以降、対象をひんし状態にした場合、攻撃が2段階ではなく3段階上昇します。（とどめばり）
#define B_KINGS_SHIELD_LOWER_ATK    GEN_LATEST // 第8世代以降、攻撃してきた相手の攻撃を2段階ではなく1段階下げます。（キングシールド）
...
#define B_BRICK_BREAK                   GEN_LATEST // 第4世代以降、自陣の壁を破壊することができます。第5世代以降では、対象がその効果を無効化する場合でも、壁は解除されません。（かわらわり）
...
#define B_DARK_VOID_FAIL                GEN_LATEST // 第7世代以降、ダークライのみが「ダークホール」を使用できます。
#define B_HIT_THAW                      GEN_LATEST // 第6世代以降、使用者のこおり状態を解除する効果を持つダメージ技は、相手のこおり状態も解除します。第3世代以降では、ほのおタイプの技は相手のこおり状態を解除します。第1・2世代では、やけど状態にする可能性のあるダメージ技は、相手がやけど状態になり得るかどうかにかかわらず、相手のこおり状態を解除します。
#define B_HEALING_WISH_SWITCH           GEN_LATEST // 第5世代以降、「いやしのねがい」や「みかづきのまい」を受けるポケモンはターンの終了時に場に出ます。さらに第8世代以降では、状態異常やダメージを受けているポケモンに交代するまで、その効果は保持されます。
...
#define B_HAZE_FOCUS_ENERGY             GEN_LATEST // 第1世代と第4世代では、「くろいきり」は「きあいだめ」の効果も解除します。
#define B_AUTOTOMIZE_FORM_CHANGE        GEN_LATEST // 第6世代以降、フォルムチェンジを行うと「ボディパージ」による体重減少効果がリセットされます。
...
#define B_TRANSFORM_CATCH_RATE          GEN_LATEST // 第3～4世代では、変身したポケモンは変身後のポケモンの捕獲率になりますが、それ以外の世代では元の捕獲率が維持されます。
#define B_TRANSFORM_BATTLE_REWARDS      GEN_LATEST // 第3～4世代では、「へんしん」したポケモンはコピー先の種族に応じた経験値と努力値を与えますが、それ以外の世代では、元の種族に応じた経験値と努力値を与えます。
#define B_TRANSFORM_FORM_CHANGES        GEN_LATEST // 第5世代以降、「へんしん」したポケモンはフォルムチェンジできません。
...
#define B_CHARGE                        GEN_LATEST // 第9世代以降、次に出す技のタイプにかかわらず、「じゅうでん」状態は解除されます。
...
#define B_TIME_OF_DAY_HEALING_MOVES     GEN_LATEST // 第2世代では、時間帯に応じて「あさのひざし」「つきのひかり」「こうごうせい」の回復量が2倍になります。また、回復量そのものも変更されています。「つきのひかり」は夕方の時間帯の影響を受けます。
...
#define B_COUNTER_MIRROR_COAT_ALLY      GEN_LATEST // 第5世代以降、味方からの攻撃は「カウンター」「ミラーコート」「メタルバースト」の判定対象に含まれません。一方、第4世代以前では、直前に受けた攻撃が味方からのものであった場合、これらの技は失敗していました。
...
#define B_SALT_CURE_DAMAGE              GEN_LATEST // 第9世代では、「しおづけ」は最大HPの1/8（はがね・みずタイプには最大HPの1/4）のダメージを与える。
...
#define B_RAGE_FIST                     GEN_LATEST // Championsでは、「ふんどのこぶし」は交代等で手持ちに戻ると威力がリセットされます。
#define B_MOVE_EFFECTS_BEFORE_MOVES     GEN_LATEST // Championsでは、別の技から「アンコール」を受けて「くちばしキャノン」を出す場合、その技は正常に機能します。
                                                   // さらに、溜めのモーションが表示されます。

// 特性設定
#define B_GALE_WINGS                    GEN_LATEST // 第7世代以降、発動にはHPが満タンである必要があります。（はやてのつばさ）
#define B_STANCE_CHANGE_FAIL            GEN_LATEST // 第7世代以降、混乱や麻痺などが原因で技を出せない場合、「バトルスイッチ」は発動しません。一方、第6世代では発動します。
#define B_SHADOW_TAG_ESCAPE             GEN_LATEST // 第4世代以降、双方が特性「かげふみ」を持つポケモンを出している場合、すべてのポケモンが交代や逃走を行えるようになりました。それ以前は、この状況下ではどちらも交代や逃走ができませんでした。
#define B_MOODY_ACC_EVASION             GEN_LATEST // 第8世代以降、「ムラっけ」で命中率と回避率を上げることはできなくなりました。
...
#define B_SYMBIOSIS_GEMS                GEN_LATEST // 第7世代以降、「きょうせい」は「ジュエル」で強化された後に持ち物を渡すようになりました。それ以前は「ジュエル」で強化される前に持ち物が渡されていたため、その持ち物の効果が適用されていました。
...
#define B_TRANSISTOR_BOOST              GEN_LATEST // 第9世代以降、「トランジスタ」によるでんきタイプの技の威力上昇率は、1.5倍ではなく1.3倍になります。
...
#define B_PROTEAN_LIBERO                GEN_LATEST // 第9世代以降、「へんげんじざい」と「リベロ」によるタイプ変化は、1回のバトルにつき1回のみ発生します。
#define B_INTREPID_SWORD                GEN_LATEST // 第9世代以降、「ふとうのけん」は、戦闘中に一度だけ攻撃を1段階上昇させます。
#define B_DAUNTLESS_SHIELD              GEN_LATEST // 第9世代以降、「ふくつのたて」は、戦闘中に一度だけ防御を1段階上昇させます。
#define B_DISGUISE_HP_LOSS              GEN_LATEST // 第8世代以降、「ばけのかわ」は最大HPの1/8を消費するようになりました。
...
#define B_BATTLE_BOND                   GEN_LATEST // 第9世代以降、「きずなへんげ」は戦闘中に一度だけ、攻撃・特攻・素早さを1段階上昇させます。
#define B_ATE_MULTIPLIER                GEN_LATEST // 第7世代以降、「〇〇スキン」はダメージを1.2倍にします。それ以外の世代では1.3倍となりますが、ノーマルスキンには倍率補正がかかりません。
...
#define B_MIRROR_ARMOR_STICKY_WEB       GEN_LATEST // 第9世代以降、「ミラーアーマー」は、たとえ「ねばねばネット」の使い手がまだ場に残っていたとしても、「ねばねばネット」による能力変化を跳ね返しません。
...
#define B_UNSEEN_FIST_PIERCING_DRILL    GEN_LATEST // 第9世代までは、「ふかしのこぶし」や「つらぬきドリル」は、相手が「まもる」状態であってもダメージを100%与えることができ、かつ「まもる」系の技に伴う接触時の追加効果も無効化します。
...
// アイテム設定
#define B_CONFUSE_BERRIES_HEAL      GEN_LATEST // 第3～6世代では、フィラのみなどのきのみはHPを1/8回復し、HPが半分になった時に発動します。第7世代ではHPを半分回復し、HPが25%になった時に発動します。第8世代ではHPを1/3回復します。
...
#define B_RETURN_STOLEN_NPC_ITEMS   GEN_LATEST // 第5世代以降、「どろぼう」や「ほしがる」ではNPCからアイテムを盗めなくなりました。
...
#define B_SOUL_DEW_BOOST            GEN_LATEST // 第6世代までは、「こころのしずく」はラティ兄妹の特攻と特防を上昇させます。一方、第7世代以降は、エスパータイプとドラゴンタイプの技の威力を上昇させる効果に変更されました。
...
#define B_DUSK_BALL_MODIFIER        GEN_LATEST // 第7世代以降、ダークボールの捕獲倍率は3.5倍ではなく3倍になっています。
#define B_QUICK_BALL_MODIFIER       GEN_LATEST // 第5世代以降、クイックボールの捕獲倍率は4倍ではなく5倍になっています。
#define B_LURE_BALL_MODIFIER        GEN_LATEST // 第8世代以降、ルアーボールの捕獲倍率は4倍です。第7世代では5倍、第6世代以前は3倍です。
...
#define B_SPORT_BALL_MODIFIER       GEN_LATEST // 第8世代以降、コンペボールの捕獲倍率は1.5倍から1倍に引き下げられました。
#define B_SAFARI_BALL_MODIFIER      GEN_LATEST // 第8世代以降、サファリボールの捕獲倍率は1.5倍から1倍に引き下げられました。
#define B_FRIEND_BALL_MODIFIER      GEN_LATEST // 第8世代以降、フレンドボールによるなつき度の上昇幅が200から150に引き下げられました。
...
#define B_TERRAIN_TYPE_BOOST        GEN_LATEST // 第8世代では、ダメージの補正は50%ではなく30%になります。（◯◯フィールド）
...
#define B_SHOW_USELESS_Z_MOVE_INFO          FALSE      // TRUEの場合、新しい世代の補助技のような追加効果を持たないZワザは、「追加効果なし」と表示されるようになります。
#define B_ANIMATE_MON_AFTER_KO              TRUE       // TRUEの場合、敵(自分・相手関わらず)が倒れた時に勝利アニメーションを表示する。
#define B_ANIMATE_MON_AFTER_FAILED_POKEBALL TRUE       // TRUEの場合、相手がボールから登場したときにアニメーションを表示します。
#define B_SHOW_DYNAMAX_MESSAGE              FALSE      // TRUEの場合、ダイマックスまたはキョダイマックスの完了後に、追加のバトルメッセージが表示されます。(原作にはメッセージはありません。)
...
// 捕獲設定
#define B_SEMI_INVULNERABLE_CATCH       GEN_LATEST // 第4世代以降では、半無敵状態（「あなをほる」や「そらをとぶ」など）にあるポケモンに対してボールを投げることはできません。
...
#define B_LOW_LEVEL_CATCH_BONUS         GEN_LATEST // 第8世代では、レベル20未満のポケモンを捕まえる際に捕獲率へのボーナスが加算されます。一方、第9世代では、このボーナスはレベル13未満のポケモンにのみ適用されます。
...
#define B_CATCH_SWAP_INTO_PARTY     GEN_LATEST // 第7世代以降、捕獲した野生のポケモンを手持ちに加えるかどうかの選択肢が表示され、代わりに別のポケモンをボックスへ送ることができます。
...
#define B_TRAINER_MON_RANDOM_ABILITY    FALSE      // これをTRUEに設定すると、トレーナーのポケモンにランダムな合法的な特性が生成されます。
...
#define B_PARTNER_MONS_MARKED_SEEN      FALSE      // TRUEの場合、ダブルバトルのパートナーがまだ遭遇したことのないポケモンを繰り出すと、そのポケモンは図鑑に「見つけたポケモン」として記録されます。
...
#define B_TERA_ORB_ALWAYS_CHARGED       FALSE      // TRUEの場合、B_FLAG_TERA_ORB_CHARGEDの状態にかかわらず、テラスタルオーブが常にチャージされた状態になります。モジュール性を高めるために使用してください。
...
#define B_NEW_ROCKS_PARTICLE            FALSE    // TRUEの場合、岩のパーティクルをアップデートされる。
#define B_NEW_LEAF_PARTICLE             FALSE    // TRUEの場合、葉っぱのパーティクルをアップデートされる。
...
#define B_NEW_CURSE_NAIL_PARTICLE       FALSE    // TRUEの場合、「のろい」のツメがアップデートされる。
#define B_NEW_BATON_PASS_BALL_PARTICLE  FALSE    // TRUEの場合、「バトンタッチ」のボールのグラフィックがアップデートされる。
#define B_NEW_MORNING_SUN_STAR_PARTICLE FALSE    // TRUEの場合、「あさのひざし」の星のパーティクルがアップデートされる。
#define B_NEW_IMPACT_PALETTE            FALSE    // TRUEの場合、基本的な「ヒット」パレットがアップデートされる。
...
//  バトルUI設定
#define B_MOVE_REARRANGEMENT_IN_BATTLE  GEN_3  //  第4世代以降、戦闘中に技の並び順を変更することはできません。
...
#define B_POOL_RULE_ITEM_CLAUSE             FALSE    // 各アイテムの選択を1回のみ許可する。
