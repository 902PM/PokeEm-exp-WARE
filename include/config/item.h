...
#define I_HEALTH_RECOVERY               GEN_LATEST  // 第7世代以降、一部の回復アイテムのHP回復量が以前とは異なります。
...
#define I_GEM_BOOST_POWER               GEN_LATEST  // 第6世代以降、「ジュエル」による威力上昇幅が50%から30%に引き下げられました。
...
// わざマシン設定
#define I_REUSABLE_TMS          FALSE        // 第5世代から第8世代では、わざマシンは消費しません。これをTRUEに設定すると、すべてのわざマシンが消費しなくなりますが、重要度を1に設定することで、特定のわざマシンを選択的に使用することもできます。
...
#define VAR_LAST_REPEL_LURE_USED    0 // この変数が設定されている場合、最後に使用した「スプレー」または「コロン」が記録され、プレイヤーには原作の「スプレー」使用確認（はい／いいえ）が表示されます。（ただし、`I_REPEL_LURE_MENU` が `TRUE` に設定されている場合を除きます）
