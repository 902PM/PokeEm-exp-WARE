...
#define OW_WHITEOUT_CUTSCENE            GEN_LATEST // 第4世代以降、全滅（ホワイトアウト）時に追加のメッセージが表示され、回復役のNPCを伴うイベントスクリプトのカットシーンが再生されます（この変更はFRLGにも存在しましたが、簡略化のため、GEN_3に設定した場合はRSEの挙動となります）。
...
// オブジェクトの影
#define OW_OBJECT_VANILLA_SHADOWS      FALSE    // FALSEの場合、オーバーワールド上のすべてのオブジェクトに影が表示されます。警告：これにより、各オブジェクトは1つではなく2つのスプライトを使用することになります。TRUEの場合、オブジェクトの影はジャンプ中のみ表示されます。
