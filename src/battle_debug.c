...
// const rom data
static const u8 sText_Ability[] = _("{JPN}とくせい");
static const u8 sText_HeldItem[] = _("{JPN}もちもの");
static const u8 sText_HoldEffect[] = _("{JPN}もちものこうか");
...
{
    {COMPOUND_STRING("{JPN}わざ"),        LIST_ITEM_MOVES},
    {sText_Ability,                   LIST_ITEM_ABILITY},
    {sText_HeldItem,                  LIST_ITEM_HELD_ITEM},
    {COMPOUND_STRING("{JPN}PP"),           LIST_ITEM_PP},
    {COMPOUND_STRING("{JPN}タイプ"),        LIST_ITEM_TYPES},
    {COMPOUND_STRING("{JPN}ステータス"),        LIST_ITEM_STATS},
    {COMPOUND_STRING("{JPN}ランク"),  LIST_ITEM_STAT_STAGES},
    {COMPOUND_STRING("{JPN}じょうたい"),      LIST_ITEM_STATUS1},
    {COMPOUND_STRING("{JPN}じょうたい2"),    LIST_ITEM_VOLATILE},
    {COMPOUND_STRING("{JPN}トラップ"),      LIST_ITEM_HAZARDS},
    {COMPOUND_STRING("{JPN}サイドじょうたい"),  LIST_ITEM_SIDE_STATUS},
    {COMPOUND_STRING("{JPN}AI"),           LIST_ITEM_AI},
    {COMPOUND_STRING("{JPN}AIひょうか"),   LIST_ITEM_AI_MOVES_PTS},
    {COMPOUND_STRING("{JPN}AIじょうほう"),      LIST_ITEM_AI_INFO},
    {COMPOUND_STRING("{JPN}AIてもち"),     LIST_ITEM_AI_PARTY},
    {COMPOUND_STRING("{JPN}そのほか"),      LIST_ITEM_VARIOUS},
    {COMPOUND_STRING("{JPN}しょうりする"),  LIST_ITEM_INSTANT_WIN},
};

static const struct ListMenuItem sStatsListItems[] =
{
    {COMPOUND_STRING("{JPN}いまのHP"), LIST_STAT_HP_CURRENT},
    {COMPOUND_STRING("{JPN}さいだいHP"),     LIST_STAT_HP_MAX},
    {COMPOUND_STRING("{JPN}こうげき"),     LIST_STAT_ATTACK},
    {COMPOUND_STRING("{JPN}ぼうぎょ"),    LIST_STAT_DEFENSE},
    {COMPOUND_STRING("{JPN}すばやさ"),      LIST_STAT_SPEED},
    {COMPOUND_STRING("{JPN}とくこう"),    LIST_STAT_SP_ATK},
    {COMPOUND_STRING("{JPN}とくぼう"),    LIST_STAT_SP_DEF},
};

static const struct ListMenuItem sStatus1ListItems[] =
{
    {COMPOUND_STRING("{JPN}ねむり"),         LIST_STATUS1_SLEEP},
    {COMPOUND_STRING("{JPN}どく"),        LIST_STATUS1_POISON},
    {COMPOUND_STRING("{JPN}やけど"),          LIST_STATUS1_BURN},
    {COMPOUND_STRING("{JPN}こおり"),        LIST_STATUS1_FREEZE},
    {COMPOUND_STRING("{JPN}まひ"),     LIST_STATUS1_PARALYSIS},
    {COMPOUND_STRING("{JPN}もうどく"),  LIST_STATUS1_TOXIC_POISON},
    {COMPOUND_STRING("{JPN}もうどくカウンタ"), LIST_STATUS1_TOXIC_COUNTER},
    {COMPOUND_STRING("{JPN}しもやけ"),     LIST_STATUS1_FROSTBITE},
};

static const struct ListMenuItem sVolatileStatusListItems[] =
{
    {COMPOUND_STRING("{JPN}こんらん"),          VOLATILE_CONFUSION},
    {COMPOUND_STRING("{JPN}ひるみ"),           VOLATILE_FLINCHED},
    {COMPOUND_STRING("{JPN}いちゃもん"),            VOLATILE_TORMENT},
    {COMPOUND_STRING("{JPN}ふんじん"),             VOLATILE_POWDER},
    {COMPOUND_STRING("{JPN}まるくなる"),        VOLATILE_DEFENSE_CURL},
    {COMPOUND_STRING("{JPN}いかり"),               VOLATILE_RAGE},
    {COMPOUND_STRING("{JPN}みちづれ"),        VOLATILE_DESTINY_BOND},
    {COMPOUND_STRING("{JPN}くろいまなざし"),   VOLATILE_ESCAPE_PREVENTION},
    {COMPOUND_STRING("{JPN}のろい"),             VOLATILE_CURSED},
    {COMPOUND_STRING("{JPN}みやぶる"),          VOLATILE_FORESIGHT},
    {COMPOUND_STRING("{JPN}ドラゴンエール"),        VOLATILE_DRAGON_CHEER},
    {COMPOUND_STRING("{JPN}きあいだめ"),        VOLATILE_FOCUS_ENERGY},
    ...
    {COMPOUND_STRING("{JPN}やどりぎ"),         VOLATILE_LEECH_SEED},
    {COMPOUND_STRING("{JPN}ロックオン"),            VOLATILE_LOCK_ON},
    {COMPOUND_STRING("{JPN}ほろびのうた"),        VOLATILE_PERISH_SONG},
    {COMPOUND_STRING("{JPN}ちいさくなる"),           VOLATILE_MINIMIZE},
    {COMPOUND_STRING("{JPN}じゅうでん"),             VOLATILE_CHARGE_TIMER},
    {COMPOUND_STRING("{JPN}ねをはる"),               VOLATILE_ROOT},
    {COMPOUND_STRING("{JPN}あくび"),               VOLATILE_YAWN},
...
{
    {COMPOUND_STRING("{JPN}まきびし"),       LIST_SIDE_SPIKES},
    {COMPOUND_STRING("{JPN}ねばねばネット"),   LIST_SIDE_STICKY_WEB},
    {COMPOUND_STRING("{JPN}どくびし"), LIST_SIDE_TOXIC_SPIKES},
    {COMPOUND_STRING("{JPN}ステルスロック"), LIST_SIDE_STEALTH_ROCK},
    {COMPOUND_STRING("{JPN}キョダイコウジン"),   LIST_SIDE_STEELSURGE},
};

static const struct ListMenuItem sSideStatusListItems[] =
{
    {COMPOUND_STRING("{JPN}リフレクター"),          LIST_SIDE_REFLECT},
    {COMPOUND_STRING("{JPN}ひかりのかべ"),     LIST_SIDE_LIGHTSCREEN},
    {COMPOUND_STRING("{JPN}しんぴのまもり"),        LIST_SIDE_SAFEGUARD},
    {COMPOUND_STRING("{JPN}しろいきり"),             LIST_SIDE_MIST},
    {COMPOUND_STRING("{JPN}おいかぜ"),         LIST_SIDE_TAILWIND},
    {COMPOUND_STRING("{JPN}オーロラベール"),      LIST_SIDE_AURORA_VEIL},
    {COMPOUND_STRING("{JPN}おまじない"),      LIST_SIDE_LUCKY_CHANT},
    {COMPOUND_STRING("{JPN}タイプなしダメージ"), LIST_SIDE_DAMAGE_NON_TYPES},
    {COMPOUND_STRING("{JPN}にじ"),          LIST_SIDE_RAINBOW},
    {COMPOUND_STRING("{JPN}ひのうみ"),      LIST_SIDE_SEA_OF_FIRE},
    {COMPOUND_STRING("{JPN}しつげん"),            LIST_SIDE_SWAMP},
};

static const struct ListMenuItem sAIListItems[] =
{
    {COMPOUND_STRING("{ENG}Check Bad Move"),             LIST_AI_CHECK_BAD_MOVE},
    {COMPOUND_STRING("{ENG}Try to Faint"),               LIST_AI_TRY_TO_FAINT},
    {COMPOUND_STRING("{ENG}Check Viability"),            LIST_AI_CHECK_VIABILITY},
    {COMPOUND_STRING("{ENG}Force Setup First Turn"),     LIST_AI_FORCE_SETUP_FIRST_TURN},
    {COMPOUND_STRING("{ENG}Risky"),                      LIST_AI_RISKY},
    {COMPOUND_STRING("{ENG}Try to 2HKO"),                LIST_AI_TRY_TO_2HKO},
    {COMPOUND_STRING("{ENG}Prefer Baton Pass"),          LIST_AI_PREFER_BATON_PASS},
    {COMPOUND_STRING("{ENG}Double Battle"),              LIST_AI_DOUBLE_BATTLE},
    {COMPOUND_STRING("{ENG}HP Aware"),                   LIST_AI_HP_AWARE},
    {COMPOUND_STRING("{ENG}Powerful Status"),            LIST_AI_POWERFUL_STATUS},
    {COMPOUND_STRING("{ENG}Negate Unaware"),             LIST_AI_NEGATE_UNAWARE},
    {COMPOUND_STRING("{ENG}Will Suicide"),               LIST_AI_WILL_SUICIDE},
    {COMPOUND_STRING("{ENG}Prefer Status Moves"),        LIST_AI_PREFER_STATUS_MOVES},
    {COMPOUND_STRING("{ENG}Stall"),                      LIST_AI_STALL},
    {COMPOUND_STRING("{ENG}Smart Switching"),            LIST_AI_SMART_SWITCHING},
    {COMPOUND_STRING("{ENG}Ace Pokémon"),                LIST_AI_ACE_POKEMON},
    {COMPOUND_STRING("{ENG}Omniscient"),                 LIST_AI_OMNISCIENT},
    {COMPOUND_STRING("{ENG}Smart Mon Choices"),          LIST_AI_SMART_MON_CHOICES},
    {COMPOUND_STRING("{ENG}Conservative"),               LIST_AI_CONSERVATIVE},
    {COMPOUND_STRING("{ENG}Sequence Switching"),         LIST_AI_SEQUENCE_SWITCHING},
    {COMPOUND_STRING("{ENG}Double Ace Pokémon"),         LIST_AI_DOUBLE_ACE_POKEMON},
    {COMPOUND_STRING("{ENG}Weigh Ability Prediction"),   LIST_AI_WEIGH_ABILITY_PREDICTION},
    {COMPOUND_STRING("{ENG}Prefer Highest Damage Move"), LIST_AI_PREFER_HIGHEST_DAMAGE_MOVE},
    {COMPOUND_STRING("{ENG}Predict Switch"),             LIST_AI_PREDICT_SWITCH},
    {COMPOUND_STRING("{ENG}Predict Incoming Mon"),       LIST_AI_PREDICT_INCOMING_MON},
    {COMPOUND_STRING("{ENG}Dynamic Func"),               LIST_AI_DYNAMIC_FUNC},
    {COMPOUND_STRING("{ENG}Roaming"),                    LIST_AI_ROAMING},
    {COMPOUND_STRING("{ENG}Safari"),                     LIST_AI_SAFARI},
    {COMPOUND_STRING("{ENG}First Battle"),               LIST_AI_FIRST_BATTLE},
};

static const struct ListMenuItem sVariousListItems[] =
{
    {COMPOUND_STRING("{JPN}HPを ひょうじ"),       VARIOUS_SHOW_HP},
    {COMPOUND_STRING("{JPN}みがわりのHP"), VARIOUS_SUBSTITUTE_HP},
    {COMPOUND_STRING("{JPN}メロメロ"),       VARIOUS_IN_LOVE},
};
...
static void PutMovesPointsText(struct BattleDebugMenu *data)
{
    ...
    AddTextPrinterParameterized(data->aiMovesWindowId, FONT_NORMAL, COMPOUND_STRING("スコア/ダメージ"), 3, 0, 0, NULL);
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        for (count = 0, j = 0; j < MAX_BATTLERS_COUNT; j++)
        {
            ...
            if ((chosenMoveIndex == i) && (gAiBattleData->chosenTarget[data->aiBattlerId] == j) && !(gAiLogicData->shouldSwitch & (1u << data->aiBattlerId)))
                AddTextPrinterParameterized3(data->aiMovesWindowId, FONT_NORMAL, 103 + count * 54, (i * 15) + 15, sTextColorTable[COLORID_RED], 0, COMPOUND_STRING("{JPN}/"));
            else
                AddTextPrinterParameterized(data->aiMovesWindowId, FONT_NORMAL, COMPOUND_STRING("{JPN}/"), 103 + count * 54, (i * 15) + 15, 0, NULL);
...
static const u8 sTextAll[] = _("{JPN}すべて");
...
{
    [HOLD_EFFECT_NONE]             = COMPOUND_STRING("{JPN}こうかなし"),
    [HOLD_EFFECT_RESTORE_HP]       = COMPOUND_STRING("{JPN}HP かいふく"),
    [HOLD_EFFECT_CURE_PAR]         = COMPOUND_STRING("{JPN}マヒ かいふく"),
    [HOLD_EFFECT_CURE_SLP]         = COMPOUND_STRING("{JPN}ねむり かいふく"),
    [HOLD_EFFECT_CURE_PSN]         = COMPOUND_STRING("{JPN}どく かいふく"),
    [HOLD_EFFECT_CURE_BRN]         = COMPOUND_STRING("{JPN}やけど かいふく"),
    [HOLD_EFFECT_CURE_FRZ]         = COMPOUND_STRING("{JPN}こおり かいふく"),
    ...
    [HOLD_EFFECT_SP_ATTACK_UP]     = COMPOUND_STRING("{JPN}とくこうアップ"),
    [HOLD_EFFECT_SP_DEFENSE_UP]    = COMPOUND_STRING("{JPN}とくぼうアップ"),
    [HOLD_EFFECT_CRITICAL_UP]      = COMPOUND_STRING("{JPN}クリティカルアップ"),
    [HOLD_EFFECT_RANDOM_STAT_UP]   = COMPOUND_STRING("{JPN}ランダムステータスアップ"),
    [HOLD_EFFECT_EVASION_UP]       = COMPOUND_STRING("{JPN}かいひりつ アップ"),
    [HOLD_EFFECT_WHITE_HERB]       = COMPOUND_STRING("{JPN}しろいハーブ"),
    [HOLD_EFFECT_MACHO_BRACE]      = COMPOUND_STRING("{JPN}きょうせいギプス"),
    [HOLD_EFFECT_EXP_SHARE]        = COMPOUND_STRING("{JPN}がくしゅうそうち"),
    [HOLD_EFFECT_QUICK_CLAW]       = COMPOUND_STRING("{JPN}せんせいのツメ"),
    [HOLD_EFFECT_FRIENDSHIP_UP]    = COMPOUND_STRING("{JPN}なかよしアップ"),
    [HOLD_EFFECT_MENTAL_HERB]      = COMPOUND_STRING("{JPN}メンタルハーブ"),
    [HOLD_EFFECT_CHOICE_BAND]      = COMPOUND_STRING("{JPN}こだわりハチマキ"),
    [HOLD_EFFECT_FLINCH]           = COMPOUND_STRING("{JPN}ひるみ"),
    [HOLD_EFFECT_DOUBLE_PRIZE]     = COMPOUND_STRING("{JPN}おまもりこばん"),
    [HOLD_EFFECT_REPEL]            = COMPOUND_STRING("{JPN}スプレー"),
    [HOLD_EFFECT_SOUL_DEW]         = COMPOUND_STRING("{JPN}こころのしずく"),
    [HOLD_EFFECT_DEEP_SEA_TOOTH]   = COMPOUND_STRING("{JPN}しんかいのキバ"),
    [HOLD_EFFECT_DEEP_SEA_SCALE]   = COMPOUND_STRING("{JPN}しんかいのウロコ"),
    [HOLD_EFFECT_CAN_ALWAYS_RUN]   = COMPOUND_STRING("{JPN}ランニングシューズ"),
    [HOLD_EFFECT_PREVENT_EVOLVE]   = COMPOUND_STRING("{JPN}かわらずのいし"),
    [HOLD_EFFECT_FOCUS_BAND]       = COMPOUND_STRING("{JPN}きあいのハチマキ"),
    [HOLD_EFFECT_LUCKY_EGG]        = COMPOUND_STRING("{JPN}しあわせのタマゴ"),
    [HOLD_EFFECT_SCOPE_LENS]       = COMPOUND_STRING("{JPN}ピントレンズ"),
    [HOLD_EFFECT_LEFTOVERS]        = COMPOUND_STRING("{JPN}たべのこし"),
    [HOLD_EFFECT_LIGHT_BALL]       = COMPOUND_STRING("{JPN}でんきだま"),
    [HOLD_EFFECT_TYPE_POWER]       = COMPOUND_STRING("{JPN}タイプわざ きょうか"),
    [HOLD_EFFECT_SHELL_BELL]       = COMPOUND_STRING("{JPN}かいがらのすず"),
    [HOLD_EFFECT_LUCKY_PUNCH]      = COMPOUND_STRING("{JPN}ラッキーパンチ"),
    [HOLD_EFFECT_METAL_POWDER]     = COMPOUND_STRING("{JPN}メタルパウダー"),
    [HOLD_EFFECT_THICK_CLUB]       = COMPOUND_STRING("{JPN}ふといホネ"),
    [HOLD_EFFECT_LEEK]             = COMPOUND_STRING("{JPN}ながねぎ"),
    [HOLD_EFFECT_CHOICE_SCARF]     = COMPOUND_STRING("{JPN}こだわりスカーフ"),
    [HOLD_EFFECT_CHOICE_SPECS]     = COMPOUND_STRING("{JPN}こだわりメガネ"),
    [HOLD_EFFECT_DAMP_ROCK]        = COMPOUND_STRING("{JPN}しめったいわ"),
    [HOLD_EFFECT_GRIP_CLAW]        = COMPOUND_STRING("{JPN}ねばりのかぎづめ"),
    [HOLD_EFFECT_HEAT_ROCK]        = COMPOUND_STRING("{JPN}あついいわ"),
    [HOLD_EFFECT_ICY_ROCK]         = COMPOUND_STRING("{JPN}つめたいいわ"),
    [HOLD_EFFECT_LIGHT_CLAY]       = COMPOUND_STRING("{JPN}ひかりのねんど"),
    [HOLD_EFFECT_SMOOTH_ROCK]      = COMPOUND_STRING("{JPN}さらさらいわ"),
    [HOLD_EFFECT_POWER_HERB]       = COMPOUND_STRING("{JPN}パワフルハーブ"),
    [HOLD_EFFECT_BIG_ROOT]         = COMPOUND_STRING("{JPN}おおきなねっこ"),
    [HOLD_EFFECT_EXPERT_BELT]      = COMPOUND_STRING("{JPN}たつじんのおび"),
    [HOLD_EFFECT_LIFE_ORB]         = COMPOUND_STRING("{JPN}いのちのたま"),
    [HOLD_EFFECT_METRONOME]        = COMPOUND_STRING("{JPN}メトロノーム"),
    [HOLD_EFFECT_MUSCLE_BAND]      = COMPOUND_STRING("{JPN}ちからのハチマキ"),
    [HOLD_EFFECT_WIDE_LENS]        = COMPOUND_STRING("{JPN}こうかくレンズ"),
    [HOLD_EFFECT_WISE_GLASSES]     = COMPOUND_STRING("{JPN}ものしりメガネ"),
    [HOLD_EFFECT_ZOOM_LENS]        = COMPOUND_STRING("{JPN}フォーカスレンズ"),
    [HOLD_EFFECT_LAGGING_TAIL]     = COMPOUND_STRING("{JPN}こうこうのしっぽ"),
    [HOLD_EFFECT_FOCUS_SASH]       = COMPOUND_STRING("{JPN}きあいのタスキ"),
    [HOLD_EFFECT_FLAME_ORB]        = COMPOUND_STRING("{JPN}かえんだま"),
    [HOLD_EFFECT_TOXIC_ORB]        = COMPOUND_STRING("{JPN}どくどくだま"),
    [HOLD_EFFECT_STICKY_BARB]      = COMPOUND_STRING("{JPN}くっつきバリ"),
    [HOLD_EFFECT_IRON_BALL]        = COMPOUND_STRING("{JPN}くろいてっきゅう"),
    [HOLD_EFFECT_BLACK_SLUDGE]     = COMPOUND_STRING("{JPN}くろいヘドロ"),
    [HOLD_EFFECT_DESTINY_KNOT]     = COMPOUND_STRING("{JPN}あかいいと"),
    [HOLD_EFFECT_SHED_SHELL]       = COMPOUND_STRING("{JPN}きれいなぬけがら"),
    [HOLD_EFFECT_QUICK_POWDER]     = COMPOUND_STRING("{JPN}スピードパウダー"),
    [HOLD_EFFECT_ADAMANT_ORB]      = COMPOUND_STRING("{JPN}こんごうだま"),
    [HOLD_EFFECT_LUSTROUS_ORB]     = COMPOUND_STRING("{JPN}しらたま"),
    [HOLD_EFFECT_GRISEOUS_ORB]     = COMPOUND_STRING("{JPN}はっきんだま"),
    [HOLD_EFFECT_ENIGMA_BERRY]     = COMPOUND_STRING("{JPN}ナゾのみ"),
    [HOLD_EFFECT_RESIST_BERRY]     = COMPOUND_STRING("{JPN}はんげんのみ"),
    [HOLD_EFFECT_POWER_ITEM]       = COMPOUND_STRING("{JPN}パワーアップのみ"),
    [HOLD_EFFECT_RESTORE_PCT_HP]   = COMPOUND_STRING("{JPN}HPをかいふく"),
    [HOLD_EFFECT_MICLE_BERRY]      = COMPOUND_STRING("{JPN}ミクルのみ"),
    [HOLD_EFFECT_CUSTAP_BERRY]     = COMPOUND_STRING("{JPN}イバンのみ"),
    [HOLD_EFFECT_JABOCA_BERRY]     = COMPOUND_STRING("{JPN}ジャポのみ"),
    [HOLD_EFFECT_ROWAP_BERRY]      = COMPOUND_STRING("{JPN}レンブのみ"),
    [HOLD_EFFECT_KEE_BERRY]        = COMPOUND_STRING("{JPN}アッキのみ"),
    [HOLD_EFFECT_MARANGA_BERRY]    = COMPOUND_STRING("{JPN}タラプのみ"),
    [HOLD_EFFECT_PLATE]            = COMPOUND_STRING("{JPN}プレート"),
    [HOLD_EFFECT_FLOAT_STONE]      = COMPOUND_STRING("{JPN}かるいし"),
    [HOLD_EFFECT_EVIOLITE]         = COMPOUND_STRING("{JPN}しんかのきせき"),
    [HOLD_EFFECT_ASSAULT_VEST]     = COMPOUND_STRING("{JPN}とつげきチョッキ"),
    [HOLD_EFFECT_DRIVE]            = COMPOUND_STRING("{JPN}カセット"),
    [HOLD_EFFECT_GEMS]             = COMPOUND_STRING("{JPN}ジュエル"),
    [HOLD_EFFECT_ROCKY_HELMET]     = COMPOUND_STRING("{JPN}ゴツゴツメット"),
    [HOLD_EFFECT_AIR_BALLOON]      = COMPOUND_STRING("{JPN}ふうせん"),
    [HOLD_EFFECT_RED_CARD]         = COMPOUND_STRING("{JPN}レッドカード"),
    [HOLD_EFFECT_RING_TARGET]      = COMPOUND_STRING("{JPN}ねらいのまと"),
    [HOLD_EFFECT_BINDING_BAND]     = COMPOUND_STRING("{JPN}しめつけバンド"),
    [HOLD_EFFECT_EJECT_BUTTON]     = COMPOUND_STRING("{JPN}だっしゅつボタン"),
    [HOLD_EFFECT_ABSORB_BULB]      = COMPOUND_STRING("{JPN}きゅうこん"),
    [HOLD_EFFECT_CELL_BATTERY]     = COMPOUND_STRING("{JPN}じゅうでんち"),
    [HOLD_EFFECT_MEGA_STONE]       = COMPOUND_STRING("{JPN}メガストーン"),
    [HOLD_EFFECT_SAFETY_GOGGLES]   = COMPOUND_STRING("{JPN}ぼうじんゴーグル"),
    [HOLD_EFFECT_LUMINOUS_MOSS]    = COMPOUND_STRING("{JPN}ひかりゴケ"),
    [HOLD_EFFECT_SNOWBALL]         = COMPOUND_STRING("{JPN}ゆきだま"),
    [HOLD_EFFECT_WEAKNESS_POLICY]  = COMPOUND_STRING("{JPN}じゃくてんほけん"),
    [HOLD_EFFECT_PRIMAL_ORB]       = COMPOUND_STRING("{JPN}ゲンシカイキ"),
    [HOLD_EFFECT_PROTECTIVE_PADS]  = COMPOUND_STRING("{JPN}ぼうごパット"),
    [HOLD_EFFECT_TERRAIN_EXTENDER] = COMPOUND_STRING("{JPN}グランドコート"),
    [HOLD_EFFECT_TERRAIN_SEED]     = COMPOUND_STRING("{JPN}シード"),
    [HOLD_EFFECT_ADRENALINE_ORB]   = COMPOUND_STRING("{JPN}ビビリだま"),
    [HOLD_EFFECT_MEMORY]           = COMPOUND_STRING("{JPN}メモリ"),
    [HOLD_EFFECT_Z_CRYSTAL]        = COMPOUND_STRING("{JPN}Zクリスタル"),
    [HOLD_EFFECT_UTILITY_UMBRELLA] = COMPOUND_STRING("{JPN}ばんのうがさ"),
    [HOLD_EFFECT_EJECT_PACK]       = COMPOUND_STRING("{JPN}だっしゅつパック"),
    [HOLD_EFFECT_ROOM_SERVICE]     = COMPOUND_STRING("{JPN}ルームサービス"),
    [HOLD_EFFECT_BLUNDER_POLICY]   = COMPOUND_STRING("{JPN}からぶりほけん"),
    [HOLD_EFFECT_HEAVY_DUTY_BOOTS] = COMPOUND_STRING("{JPN}あつぞこブーツ"),
    [HOLD_EFFECT_THROAT_SPRAY]     = COMPOUND_STRING("{JPN}のどスプレー"),
    [HOLD_EFFECT_ABILITY_SHIELD]   = COMPOUND_STRING("{JPN}とくせいガード"),
    [HOLD_EFFECT_CLEAR_AMULET]     = COMPOUND_STRING("{JPN}クリアチャーム"),
    [HOLD_EFFECT_MIRROR_HERB]      = COMPOUND_STRING("{JPN}ものまねハーブ"),
    [HOLD_EFFECT_PUNCHING_GLOVE]   = COMPOUND_STRING("{JPN}パンチグローブ"),
    [HOLD_EFFECT_COVERT_CLOAK]     = COMPOUND_STRING("{JPN}おんみつマント"),
    [HOLD_EFFECT_LOADED_DICE]      = COMPOUND_STRING("{JPN}いかさまダイス"),
    [HOLD_EFFECT_BOOSTER_ENERGY]   = COMPOUND_STRING("{JPN}ブーストエナジー"),
    [HOLD_EFFECT_OGERPON_MASK]     = COMPOUND_STRING("{JPN}オーガポンのおめん"),
    [HOLD_EFFECT_BERSERK_GENE]     = COMPOUND_STRING("{JPN}はかいのいでんし"),
};
